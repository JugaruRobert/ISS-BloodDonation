...
namespace BloodDonation
{
    public partial class Form1 : Form
    {
        ...
        private void DoctorPopulateDonations()
        {
        ...
        private void DonorPopulateDonations()
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            seeNotificationPanel.Visible = false;
            DonorPopulateDonations();
        ...
        private void registerFormButton_Click(object sender, EventArgs e)
        {
            ...
            registerAddressCityBox.BackColor = Color.White;
            registerResidenceCityBox.BackColor = Color.White;
            ...
            try
            {
                ...
                if (registerAddressCityBox.SelectedIndex == -1 || registerAddressCityBox.SelectedIndex == 0)
                    drops.Add(registerAddressCityBox);

                if (registerResidenceCityBox.SelectedIndex == -1 || registerResidenceCityBox.SelectedIndex == 0)
        ...
        private void requestFormButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (redBloodCellsQuantity == 0 && plasmaQuantity == 0 && trombocitesQuantity == 0)
                {
                    boxes.Add(requestRedCellsBox);
                    boxes.Add(requestPlasmaBox);
                    boxes.Add(requestTrombocitesBox);
        ...
        private void registerResidenceCountryBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(registerResidenceCountryBox.SelectedIndex != 0)
            {
                ...
                {
                    registerResidenceCityBox.Items.Add("City");
        ...
        private void registerAddressCountryBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (registerAddressCountryBox.SelectedIndex != 0)
            {
                ...
                {
                    registerAddressCityBox.Items.Add("City");
                    foreach (City city in cities)
                    {
        ...
        private void donationsToTestDGV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Donor donor = context.DonorController.ReadByID(globalBloodDonation.DonorCNP);
            if(donor.BloodTypeID == new Guid())
            {
                bloodTypeLabel.Visible = true;
                bloodTypeCombo.Visible = true;
                FillBloodTypes();
            }
            else
            {
                bloodTypeLabel.Visible = false;
                bloodTypeCombo.Visible = false;
            }
        }

        private void FillBloodTypes()
        {
            bloodTypeCombo.Items.Clear();
            foreach(KeyValuePair<Guid, string> b in bloodTypes)
            {
                bloodTypeCombo.Items.Add(b.Value);
            }
            if(bloodTypes.Count() != 0)
                bloodTypeCombo.SelectedIndex = 0;
        }
        ...
        {
            textBox1.BackColor = Color.White;
            bloodTypeCombo.BackColor = Color.White;
            ...
            {
                if(bloodTypeCombo.Visible == true && (bloodTypeCombo.SelectedIndex == 0 || bloodTypeCombo.SelectedIndex == -1))
                {
                    bloodTypeCombo.BackColor = Color.IndianRed;
                }
                else
                {
                    int quantity = 0;

                    if (bloodTypeCombo.Visible == true && bloodTypeCombo.SelectedIndex != 0)
                    {
                        string type = bloodTypeCombo.SelectedItem.ToString();
                        Guid bloodID = bloodTypes.FirstOrDefault(x => x.Value == type).Key;

                        Donor donor = context.DonorController.ReadByID(globalBloodDonation.DonorCNP);
                        donor.BloodTypeID = bloodID;
                        context.DonorController.Update(donor);
                    }
                    ...
                    if (diseasesCheckBox.CheckedItems.Count != 0)
                    {
                        ...
                        if (result == true)
                        {
                            String text = "Tests are completed.They were positive for some diseases.";
                            ...
                            Notification notification = new Notification(Guid.NewGuid(), this.globalBloodDonation.DonorCNP, text, "Not Read");
                            context.NotificationController.Insert(notification);

                            DoctorPopulateDonations();
        ...
        private void notificationsDGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(notificationsDGV.SelectedCells.Count > 0)
            {
                int row = notificationsDGV.SelectedCells[0].RowIndex;
                Guid notificationID = new Guid(notificationsDGV.Rows[row].Cells[2].Value.ToString());
                Notification notification = context.NotificationController.ReadByID(notificationID);
                notification.Status = "Read";
                context.NotificationController.Update(notification);
                notificationText.Text = notification.Description
                    .Split('.')
                    .Select(n => n.Trim().Split(',').Select(s => s.Trim()).Aggregate((s1, s2) => s1 + '\n' + s2))
                    .Aggregate((p1, p2) => p1 + '\n' + p2);
                seeNotificationPanel.Visible = true;
                List<Notification> notifications = context.NotificationController.ReadAll().ToList();
                notifications = notifications.Where(n => n.DonorCNP == donor.CNP).ToList();
                List<String> donorNotifications = new List<String>();
                Dictionary<string, bool> columns = new Dictionary<string, bool>();
        ...
        private void notificationBackButton_Click(object sender, EventArgs e)
        {
            seeNotificationPanel.Visible = false;
